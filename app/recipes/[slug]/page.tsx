...
import { fetchPublicAdSettings } from '@/lib/supabase/fetch-site-settings';
import { AdSensePageBootstrap } from '@/components/ads/adsense-page-bootstrap';
...
export default async function RecipeDetailPage({
  ...
  const recipe = localizeRecipe(recipeData as Recipe & { category: Category });
  ...
  const relatedRecipes =
    (relatedData as (Recipe & { category: Category })[] | null)?.map((r) =>
      localizeRecipe(r)
    ) ?? [];

  const adSettings = await fetchPublicAdSettings(supabase);
  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      />
      <div className="w-full px-4 pt-2 sm:px-6 lg:px-8 print:hidden">
        <div className="mx-auto max-w-3xl">
          <AdSensePlacement settings={adSettings} placementKey="below_header" />
        </div>
      </div>
      <main className="flex-1">
        <article className="recipe-editorial-page py-10 sm:py-14">
          <div className="mx-auto max-w-[730px] px-4 sm:px-6 lg:px-8">
            ...
            <AdSensePlacement settings={adSettings} placementKey="recipe_after_intro" />

            <RecipeBlog html={recipe.blog} heading={rc.storyHeading} />
            ...
            <div className="oc-recipe-container overflow-hidden">
              <header className="oc-recipe-header space-y-5 px-4 pb-2 pt-6 text-center sm:space-y-6 sm:px-8 sm:pt-8">
                ...
                <div className="oc-recipe-info flex flex-wrap items-center justify-center gap-x-6 gap-y-2 px-2 font-sans text-[0.9375rem]">
                  {recipe.prep_time != null && recipe.prep_time > 0 && (
                    <span>
                      <span className="oc-recipe-meta-label">{rc.metaPrep}</span>{' '}
                      <span className="font-semibold tabular-nums text-foreground">
                        {recipe.prep_time} {rc.minutes}
                      </span>
                    </span>
                  )}
                  {recipe.cook_time != null && recipe.cook_time > 0 && (
                    <span>
                      <span className="oc-recipe-meta-label">{rc.metaCook}</span>{' '}
                      <span className="font-semibold tabular-nums text-foreground">
                        {recipe.cook_time} {rc.minutes}
                      </span>
                    </span>
                  )}
                  {totalTime > 0 && (
                    <span>
                      <span className="oc-recipe-meta-label">{rc.totalTime}:</span>{' '}
                      <span className="font-semibold tabular-nums text-foreground">
                        {totalTime} {rc.minutes}
                      </span>
                    </span>
                  )}
                  {recipe.difficulty && (
                    <span>
                      <span className="oc-recipe-meta-label">{rc.difficulty}:</span>{' '}
              ...
              <section className="oc-recipe-content space-y-11 px-4 py-8 sm:space-y-12 sm:px-8 sm:pb-11">
                <div className="wprm-recipe-ingredients-wrap">
                  <h3 className="oc-recipe-section-title text-left">{rc.ingredients}</h3>
                ...
                <AdSensePlacement settings={adSettings} placementKey="recipe_between_sections" />

                <div className="wprm-recipe-instructions-wrap">
                  <h3 className="oc-recipe-section-title text-left">{rc.instructions}</h3>
                ...
                {recipe.notes?.trim() ? (
                  <div className="wprm-recipe-notes-wrap border-t border-[#535e50]/12 pt-9 dark:border-border">
                    <h3 className="oc-recipe-section-title text-left">{rc.notes}</h3>
      ...
      <div className="w-full px-4 pb-4 sm:px-6 lg:px-8 print:hidden">
        <div className="mx-auto max-w-3xl">
